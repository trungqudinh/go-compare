...
func testValue(t *testing.T, a, b, c, array interface{}, desc string) {
	test(t, EqualTo(a, b), desc+" values are not equal, must be equal")
	test(t, !EqualTo(a, c), desc+" values are equal, must be not equal")
	test(t, LessThan(a, c), desc+" value A is not less than value B, must be less")
	test(t, LessOrEqual(a, b), desc+" value A is not less or equal than value B, must be less or equal")
	test(t, LessOrEqual(a, c), desc+" value A is not less or equal than value B, must be less or equal")
	test(t, GreaterOrEqual(a, b), desc+" value A is not greater or eqail than value B, must be greater or equal")
	test(t, GreaterOrEqual(c, a), desc+" value B is not greater or eqail than value A, must be greater or equal")
	test(t, GreaterThan(c, a), desc+" value B is not greater than value A, must be greater")
}

func TestAsserts(t *testing.T) {
	test(t, EqualTo(nil, nil), "Nil values are not equal, must be equal")
	float64A := 2.24
	float64B := 2.24
	float64C := 8.0
	...
	boolA := true
	boolB := true
	test(t, EqualTo(boolA, boolB), "Bool values are not equal, must be equal")
	boolB = false
	test(t, !EqualTo(boolA, boolB), "Bool values are equal, must be not equal")
}
